...
# Show log messages starting from warning, or set IP-SCANNER-LOG-LEVEL env var
# (https://docs.python.org/3/library/logging.html#levels)
logging.basicConfig(level=os.environ.get('IP-SCANNER-LOG-LEVEL', 'DEBUG'))
...
class ScanWebServer():
    ...
    def __init__(self, ips: list, scan_software: bool = True, scan_root: bool = True, preserve_ips = True) -> None:
        self.ips = ips
        self.scan_software = scan_software
        self.scan_root = scan_root
        self.preserve_ips = preserve_ips
        self.ip_map: IP_MAP_TYPE = {}
        logging.debug(f'ScanWebServer() called with {locals()}.')
    ...
    def _server_software(self, resp: requests.Response) -> WEBSRVENUM:
        ...
        if not server_field:
            logging.warning('No server field in HTTP header.')
            return WEBSRVENUM.none
        logging.debug(f'HTTP "server" header field is set to: {server_field}')
        try:
            software = WEBSRVENUM(server_field.split('/')[0].lower())
        except ValueError:
            logging.warning(f'"{server_field}" is unsupported.')
            software = WEBSRVENUM.other
        logging.debug(f'Server type is set to: {software}')
